...
- 来源声明（2026-06-25，GG）：AI 联网核查启动、进度刷新、富文本结果和 Markdown 报告顶部会由程序直接插入“基于视频嚼真机开源项目：https://github.com/foddcus/Bilibili-Infochcker”，该内容不写入 DeepSeek 提示词，也不要求 AI 生成。
...
- AI 联网评价报告保存到运行目录 `输出文本/AI评价` 子文件夹，同一次评价会保存 `.md` 阅读版和 `.json` 结构化版；界面 AI 鉴别区和 Markdown 报告会在 AI 评价内容前显示固定开源项目来源声明，关键主张会在主张后显示五级评价短标签。
...
- 主界面标题已更新为“视频嚼真机 V1.1 - 听完再验”，界面显示可点击产品署名 `designed by foddcus 快怿`，点击打开 `https://github.com/foddcus`。
- 工程版本号已写入 V1.1：`AudioText.App.csproj` 设置 `Version=1.1.0`、`AssemblyVersion=1.1.0.0`、`FileVersion=1.1.0.0` 和 `InformationalVersion=V1.1`，并关闭信息版本号自动追加 git 修订号；主窗口标题、首页主标题和启动日志同步显示 `V1.1`。
...
- V1.1 新增本地记忆数据库（2026-06-25，GG）：`AudioText.Infrastructure/Services/VideoAnalysisMemoryStore.cs` 使用无外部 NuGet 依赖的 JSON 文件 `记忆数据库/video-analysis-memory.json` 保存来源链接、标题、平台、发布人、转写文本和最近一次 AI 评分；创建任务时会先按来源链接查库，重复视频直接复用已转写文本并重新 AI 核查；AI 核查前会加载同一发布人的最多 5 条往期视频摘要作为背景参考，但提示词明确要求历史记忆不能替代本次联网证据。
- V1.1 新增自动清理（2026-06-25，GG）：任务分析流程结束后，主窗口只删除软件托管的 `下载音频/` 中间文件和 `_临时下载/` 子目录，不删除 `输出文本/`、`输出文本/AI评价/` 或 `记忆数据库/`，避免重复分析失去已转写数据。
- V1.1 新增彩色五级主张标签（2026-06-25，GG）：AI 评语栏中“客观属实 / 基本属实 / 有失偏颇 / 煽风点火 / 胡言乱语”分别使用绿色、黄绿色、琥珀色、橙色和红色显示，Markdown 和 JSON 仍保留纯文本结构，便于后续复用。
